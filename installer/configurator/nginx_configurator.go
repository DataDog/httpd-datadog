...
func (n *NginxConfigurator) ModifyConfig(remoteConfigID, appID, site, clientToken, agentUri string, sessionSampleRate, sessionReplaySampleRate int, dryRun bool) error {
	...
	newContent, err := transformConfig(n, content, remoteConfigID, agentUri, appID, clientToken, site, sessionSampleRate, sessionReplaySampleRate, newConfigPath)
...
func transformConfig(n *NginxConfigurator, content []byte, remoteConfigID, agentUri string, appID string, clientToken string, site string, sessionSampleRate int, sessionReplaySampleRate int, destFile string) ([]byte, error) {
    ...
    datadog_rum_config "v6" {
        "applicationId" "%s";
        "clientToken" "%s";
        "site" "%s";`, agentUri, appID, clientToken, site)
	if remoteConfigID != "" {
		datadogConfig += fmt.Sprintf(`
        "remoteConfigurationId" "%s";`, remoteConfigID)
	} else {
		datadogConfig += fmt.Sprintf(`
        "sessionSampleRate" "%d";
        "sessionReplaySampleRate" "%d";`, sessionSampleRate, sessionReplaySampleRate)
	}
